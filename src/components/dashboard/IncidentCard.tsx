...
import { ShieldAlert, HeartPulse, Flame, Car, Eye, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
const getIncidentIcon = (type: Incident['type']) => {
  switch (type) {
    ...
    case 'Medical Emergency': return <HeartPulse className="h-5 w-5 text-blue-400" />; // Specific blue for medical
    case 'Fire Alert': return <Flame className="h-5 w-5 text-orange-500" />; // Specific orange for fire
    case 'Traffic Accident': return <Car className="h-5 w-5 text-yellow-500" />;
    case 'Suspicious Activity': return <Eye className="h-5 w-5 text-purple-400" />; // Specific purple for suspicious
